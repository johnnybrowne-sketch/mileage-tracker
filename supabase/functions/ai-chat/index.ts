...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (!response.ok || json.error) {
      ...
      throw new Error(json.error?.message || "Claude help assistant failed.");
    }
...
function buildSystemPrompt({
  ...
  return `
You are Prosper Real Estate's friendly Mileage Tracker helper.
...
- Paper Sheets allows upload of mileage forms. Claude scans photos/PDFs into editable draft rows. Workers must review and fix flagged rows before submitting.
- Claude flags unreadable handwriting as "not readable" and needs review.
