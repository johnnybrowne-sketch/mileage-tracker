...
import { expandFleetVehicleOptions } from "../services/fleetVehicleService";
import {
...
const OTHER_COMPANY_VEHICLE_VALUE = "__other_company_vehicle__";
...
export default function AdminDashboard() {
  ...
  async function refreshAllRealtimeData(options = {}) {
    ...
    setVehicles(expandFleetVehicleOptions(vehiclesResult.data || []));
    ...
    setAddForm((currentForm) => {
      ...
      if (field === "vehicleName") {
        ...
        const effectiveVehicleName =
          value === OTHER_COMPANY_VEHICLE_VALUE
        ...
        nextForm.startOdometer = effectiveVehicleName
          ? String(odometerStart.expectedStartOdometer || "")
          : "";
        nextForm.expectedStartOdometer = effectiveVehicleName
          ? String(odometerStart.expectedStartOdometer || "")
          : "";
        nextForm.usesSharedVehicleOdometer = effectiveVehicleName
          ? odometerStart.isSharedVehicle
          : false;
        nextForm.customVehicleName =
      ...
      if (
        ...
        currentForm.vehicleName === OTHER_COMPANY_VEHICLE_VALUE
      ) {
        const odometerStart = getExpectedVehicleStart({
          states: vehicleOdometerStates,
          vehicle: null,
  ...
  async function handleAddEntry(event) {
    ...
    try {
      ...
      setSelectedVehicle(effectiveVehicleName);
  ...
  async function handleCompleteTimesheetMileage(event) {
    ...
    const effectiveVehicleName = getAdminFormVehicleName(timesheetMileageForm);
          ...
          <div className="mt-5 grid gap-4 lg:grid-cols-2 2xl:grid-cols-4">
            ...
            <FormField label="Vehicle">
              ...
              >
                ...
                <option value={OTHER_COMPANY_VEHICLE_VALUE}>
                  Other company vehicle
                </option>
              </select>
            </FormField>

            {addForm.vehicleName === OTHER_COMPANY_VEHICLE_VALUE && (
              <FormField label="Other Vehicle Name">
                <input
                  type="text"
                  required
                  value={addForm.customVehicleName}
                  onChange={(event) =>
                    updateAddForm("customVehicleName", event.target.value)
                  }
      ...
      {selectedTimesheet && (
        ...
        >
          ...
          <div className="mt-5 grid gap-4 lg:grid-cols-3">
            ...
            {mileageForm.vehicleName === OTHER_COMPANY_VEHICLE_VALUE && (
              <FormField label="Other Vehicle Name">
                <input
                  type="text"
                  required
...
function JobberMileageCell({ entry, timesheet, properties = [] }) {
  ...
  const resolvedPropertyCode = getResolvedEntryPropertyCode(
    entry,
    timesheet,
    properties
  );
...
function getMileagePropertyCodeValue({ form, property, jobberVisit, properties = [] }) {
  if (jobberVisit) {
    return resolvePropertyCode({
      address: jobberVisit.jobberPropertyAddress,
      properties,
      fallbackCode: getJobberPropertyCode(jobberVisit) || form.propertyCode,
    });
  }
...
function getAdminFormVehicleName(form) {
  if (form?.vehicleName === OTHER_COMPANY_VEHICLE_VALUE) {
...
function getLatestEndOdometerForWorkerVehicle({
  ...
  const sortedEntries = [...entries].sort((a, b) => {
    const dateA = (parseMileageDate(getEntryDate(a))?.getTime() || 0);
    const dateB = (parseMileageDate(getEntryDate(b))?.getTime() || 0);
    return dateB - dateA;
  });
...
function buildCompleteMileageEntryUpdatePayload({
  ...
  setExistingMileageWorkflowColumns({
    payload,
    row,
    form,
    jobberVisit,
...
function setExistingMileageWorkflowColumns({ payload, row, form, jobberVisit }) {
  const workflowColumns = buildMileageWorkflowFields({
    jobberVisit,
  ...
  Object.entries(workflowColumns).forEach(([columnName, value]) => {
    if (hasOwnColumn(row, columnName)) {
      payload[columnName] = value;
    }
  });
...
function buildMileagePayloadForSchema({
  ...
  setMileageWorkflowPayloadColumns({
    payload,
    sampleEntry,
    hasSampleEntry,
    form,
...
function setMileageWorkflowPayloadColumns({
  payload,
  sampleEntry,
  hasSampleEntry,
  form,
